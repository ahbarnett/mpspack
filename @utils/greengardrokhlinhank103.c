/* MEX interface to Greengard-Rokhlin fortran codes for Hankels.

   Must be linked to hank103.o which is compiled by gfortran hank103.f -c -O3
   Compilation in matlab:
     mex greengardrokhlinhank103.c hank103.o LD=gfortran
   See Makefile
                 ...
                 int nrhs, const mxArray *prhs[])
{
  ...
  int ifexpon = 1;     /* if 1 unscaled; 0 hank103 scales by  e^{-i \cdot z} */

  if (nrhs != 1)
    mexErrMsgTxt("greengardrokhlinhank103: must have one input argument");
  if (nlhs > 2)
    mexErrMsgTxt("greengardrokhlinhank103: must have no more than two output arguments");
  if (mxIsClass(prhs[0],"sparse") || mxIsChar(prhs[0]))
    mexErrMsgTxt("greengardrokhlinhank103: input must be full and nonstring");
  ...
  if (!mxIsComplex(prhs[0]))            /* input is real, has no imag part */
    for (j=0;j<n*m;++j) {               /* note looping over array in one go */
      ...
      hank103_(z, H0, H1, &ifexpon);    /* pass by reference to fortran */
