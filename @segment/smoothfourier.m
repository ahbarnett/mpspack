function s = smoothfourier(M, aj, bj, varargin)
...
%
% s = SMOOTHFOURIER(M, aj, bj, opts) allowed control of options:
%   opts.napproxv = number of approximating vertices, see segment.
...
if n==0, s = segment.radialfunc(M, {@(t) 1+0*t, @(t) 0*t, @(t) 0*t}, varargin{:});
  ...
  s = segment.radialfunc(M, {@(t) 1 + reshape(aj*cos(j*t(:).') + bj*sin(j*t(:).'),size(t)), ...
                      ...
                      @(t) reshape(-(j'.^2.*aj)*cos(j*t(:).') - (j'.^2.*bj)*sin(j*t(:).') ,size(t))}, varargin{:});
end
