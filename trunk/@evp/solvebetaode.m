...
%
% kh = SOLVEBETAODE(kstar, beo, A, B) solves the ODE
%   db/dk = (1 - B.b + A.b^2)/k,    with IC b(kstar) = beo,
%   returning the wavenumber value kh at which b(kh) = 0.
%
%   This is used to give a better khat prediction.
%   kstar = starting k, beo = starting beta,
%   A=kstar^2.||x_t f||^2 - ||x_t f_t||^2,  B = Re<f,mf>, kh = predicted \hat{k}
%
% kh = SOLVEBETAODE(kstar, beo, A, B, C) solves the ODE
%   db/dk = (1 - Bb + (k^2.C - A)b^2)/k,    with IC b(kstar) = beo,
%   returning the wavenumber value kh at which b(kh) = 0.
%
%   This is a slightly different way to better khat prediction.
...
% Frozen kstar 10/5/11

% x = k - kstar, is indep var of ODE (more accurate than using k):
if nargin==4
  F = @(x,b) (1 - B*b + A.*b.*b)./(kstar+x);
elseif nargin==5
  F = @(x,b) (1 - B*b + ((kstar+x).^2*C-A).*b.*b)./(kstar+x);
elseif nargin==8
  F = @(x,b) (1 - (B + Bp*x).*b + ((kstar+x).^2*(C+Cp*x)-(A+Ap*x)).*b.*b)./(kstar+x); % Alex linear approx
end
