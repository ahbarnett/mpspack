...
unpack this with
\co{tar zxvf mpspack-1.31.tar.gz}
...
As of version 1.31, \mpspack\ uses a point-in-polygon checker by Bruno Luong for which MEX executables are provided.
If these fail (running {\tt test/testdomain.m} is sufficient),
then you should edit the file {\tt @utils/inpolywrapper.m}
to use one of the other available checkers: either those compiled by the
above {\em make}, or the native \matlab\ {\tt inpolygon} routine
(which is very slow, but will get you going).
