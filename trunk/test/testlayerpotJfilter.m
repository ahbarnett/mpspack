...
p = segment(52, [orig, R, 0, 2*pi], 'p');          % circle of target pts
if verb>1
...
o.Jfilter.M = 50;
o.Jfilter.rescale_rad = 0.8;      % radius to scale J-exp coeffs to be O(1) at
o.Jfilter.origin = orig;    % test displaced origin
...
f = @(x) 3*sin(3*real(x)+1); %1+0.5*real(x);          % choose a (smooth) density function
...
c = (.3+(dens=='d'))/sqrt(1+d.k); % plot segment opts, color axis
if verb>0
